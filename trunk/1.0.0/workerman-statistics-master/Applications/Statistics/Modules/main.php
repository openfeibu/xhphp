...
EOT;
    ...
    $module_arr = $allModules = $statistic_arr = array();
    foreach(\Statistics\Lib\Cache::$modulesDataCache as $mod => $interfaces)
    {
        ...
        $module_arr[] .= $mod; 
        $allModules[$mod]= allMultiRequestStAndInterfaces($mod,$interfaces ,$date);
    }
   
    $categories = '[';
    foreach ($allModules as $key => $module) {
        $module_total_count = 0;
        foreach ($module as $k => $interface) {
            $statisticDataCache = allMutiRequestStAndStatisticDataCache($key,$interface ,$date);
           
            $all_st_str = '';           
            if($statisticDataCache['statistic'])
            {
                foreach($statisticDataCache['statistic'] as $ip=>$st_str)
                {
                    $all_st_str .= $st_str;
                }
            }
            $code_map = array();
            $data = formatSt($all_st_str, $date, $code_map);
            $interface_total_count = $fail_count = 0;
            foreach($data as $time_point=>$item)
            {
                if($item['total_count'])
                {
                    $interface_total_count += $item['total_count'];
                }
                $fail_count += $item['fail_count'];
            }
            $module_total_count += $interface_total_count;
            $statistic_arr[$key]['interface'][$interface]['total_count'] =  $interface_total_count;
        }
        
        $statistic_arr[$key]['total_count'] = $module_total_count;
        $categories .= "'".$key."',";
    }
...
function allMultiRequestStAndInterfaces($module, $interface, $date)
{   
    $allModules = array();
    $buffer = json_encode(array('cmd'=>'get_statistic','module'=>$module, 'interface'=>$interface, 'date'=>$date))."\n";
    $ip_list = (!empty($_GET['ip']) && is_array($_GET['ip'])) ? $_GET['ip'] : \Statistics\Lib\Cache::$ServerIpList;
...
function allMutiRequestStAndStatisticDataCache($module, $interface, $date)
{
    $statisticDataCache['statistic'] = '';
    $buffer = json_encode(array('cmd'=>'get_statistic','module'=>$module, 'interface'=>$interface, 'date'=>$date))."\n";
    $ip_list = (!empty($_GET['ip']) && is_array($_GET['ip'])) ? $_GET['ip'] : \Statistics\Lib\Cache::$ServerIpList;
    $reqest_buffer_array = array();
    $port = \Statistics\Config::$ProviderPort;;
    ...
    foreach($read_buffer_array as $address => $buf)
    {
        ...
        $statisticDataCache['statistic'][$ip] = $statistic_data;
    }
    return $statisticDataCache;
}
